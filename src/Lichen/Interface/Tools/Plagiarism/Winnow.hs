module Lichen.Interface.Tools.Plagiarism.Winnow where
...
hashWin l = Tagged (hash . fmap tdata $ l) (spanPos . fmap tpos $ l) where
    spanPos lst = TokenPosition (startLine $ NE.head lst) (endLine $ NE.last lst) (startCol $ NE.head lst) (endCol $ NE.last lst)
